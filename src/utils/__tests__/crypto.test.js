...
describe('crypto', () => {
    ...
    describe('isEncrypted', () => {
        ...
        test('平文を正しく識別する', () => {
            ...
            expect(isEncrypted({ ciphertext: 'test' })).toBe(false);
        });
    });
    ...
    describe('encryptApiKey and decryptApiKey', () => {
        ...
        test('無効なAPIキーでエラーをスローする', async () => {
            ...
            await expect(encryptApiKey(123, key)).rejects.toThrow('Invalid API key');
        });
        ...
        test('無効な暗号化データでエラーをスローする', async () => {
            ...
            await expect(decryptApiKey({}, key)).rejects.toThrow('Invalid API key format');
        });
    });
});