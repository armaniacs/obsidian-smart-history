import { getSettings, StorageKeys, getAllowedUrls, Settings } from '../utils/storage.js';
...
export class AIClient {
    ...
    async generateSummary(content: string): Promise<string> {
        ...
        const providerName = (settings as any)[StorageKeys.AI_PROVIDER] || 'gemini';
     ...
     */
    async generateGeminiSummary(content: string, apiKey: string, modelName: string): Promise<string> {
        if (!apiKey) {
     ...
     */
    async generateOpenAISummary(content: string, baseUrlRaw: string, apiKey: string, modelNameRaw: string): Promise<string> {
    ...
    async listGeminiModels(apiKey: string): Promise<string> {
        try {
            ...
            const data = await response.json();
            return data.models ? data.models.map((m: any) => m.name).join(', ') : "No models returned";
        } catch (e: any) {
