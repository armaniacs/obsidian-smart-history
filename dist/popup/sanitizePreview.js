...
let previewTrapId = null; // フォーカストラップID
...
function jumpToMaskedPosition(index) {
    const previewContent = getPreviewContent();
    if (!previewContent || maskedPositions.length === 0)
        return;
...
function buildMaskNavigation(container) {
    ...
    if (maskedPositions.length > 0) {
        ...
        const counter = document.getElementById('maskNavCounter');
        if (counter)
            counter.textContent = `0/${maskedPositions.length}`;
    }
    else {
...
//# sourceMappingURL=sanitizePreview.js.map