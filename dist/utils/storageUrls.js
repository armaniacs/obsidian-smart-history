 ...
 */
export function buildAllowedUrls(settings, isDomainInWhitelistFunc) {
    const allowedUrls = new Set();
    // Obsidian API
    const protocol = settings.obsidian_protocol || 'https';
    const port = settings.obsidian_port || '27124';
    ...
    const openaiBaseUrl = settings.openai_base_url;
    if (openaiBaseUrl) {
    ...
    const openai2BaseUrl = settings.openai_2_base_url;
    if (openai2BaseUrl) {
    ...
    const ublockSources = settings.ublock_sources || [];
 ...
 */
export async function saveSettingsWithAllowedUrls(settings, saveSettingsFunc) {
 ...
 */
export async function getAllowedUrls(ALLOWED_URLS_KEY) {
    const result = await chrome.storage.local.get(ALLOWED_URLS_KEY);
    const urls = result[ALLOWED_URLS_KEY] || [];
    return new Set(urls);
}
/**
