 ...
 */
export function calculateTextScore(element) {
    let score = 0;
    // テキストノードの長さ
    ...
    const walker = document.createTreeWalker(element, NodeFilter.SHOW_ELEMENT, undefined);
    ...
    while (node) {
        ...
        if (tag === 'p') {
            ...
            linkCount++;
            linkTextLength += elem.innerText?.length || elem.textContent?.length || 0;
        }
 ...
 * メインコンテンツの候補要素を抽出
 */
function findMainContentCandidates() {
    const candidates = [];
    ...
    // body直下の子要素を候補にする
    const directChildren = Array.from(body.children).filter(child => !isExcludedElement(child));
 ...
 */
export function extractMainContent(maxChars = 10000) {
    let content = '';
    try {
