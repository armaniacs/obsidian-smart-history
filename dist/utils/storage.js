...
let cachedMasterPassword = null; // セッション中のマスターパスワードキャッシュ
let cachedServerKey = null; // 【マイグレーション用】サーバーサイド保存のキー
...
export async function getOrCreateEncryptionKey() {
    ...
    const masterPasswordEnabled = result[StorageKeys.MASTER_PASSWORD_ENABLED];
    const isLocked = result[StorageKeys.IS_LOCKED];
    if (masterPasswordEnabled) {
        ...
        // マスターパスワードからキーを導出
        const passwordSaltBase64 = result[StorageKeys.MASTER_PASSWORD_SALT];
 ...
 */
export async function isMasterPasswordEnabled() {
 ...
 */
export async function isEncryptionLocked() {
...
export async function unlockWithPassword(password) {
    ...
    const enabled = result[StorageKeys.MASTER_PASSWORD_ENABLED];
    if (!enabled) {
        throw new Error('Master password not enabled');
    }
    const storedHash = result[StorageKeys.MASTER_PASSWORD_HASH];
    const saltBase64 = result[StorageKeys.MASTER_PASSWORD_SALT];
 ...
 */
export function getDomainFilterCacheSync(callback) {
    chrome.storage.local.get([
        StorageKeys.DOMAIN_FILTER_CACHE,
        StorageKeys.DOMAIN_FILTER_CACHE_TIMESTAMP,
        StorageKeys.DOMAIN_FILTER_MODE
    ], (result) => {
        const allowedDomains = result[StorageKeys.DOMAIN_FILTER_CACHE] || [];
        const cachedAt = result[StorageKeys.DOMAIN_FILTER_CACHE_TIMESTAMP] || 0;
        const mode = result[StorageKeys.DOMAIN_FILTER_MODE] || 'disabled';
 ...
 */
export function isDomainFilterCacheValid(cachedAt) {
    const now = Date.now();
 ...
 */
export function normalizeDomainUrl(url) {
    try {
 ...
 */
export function matchesWildcardPattern(domain, pattern) {
    if (pattern.includes('*')) {
        // ワイルドカードパターンを正規表現に変換
 ...
 * @param {Settings} settings - 設定オブジェクト
 */
export async function updateDomainFilterCache(settings) {
